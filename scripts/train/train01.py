...
def build_model(cfg):
    ...
    if method == "lora":
        ...
        peft_config = AdaLoraConfig(
            ...
            # AdaLoRA 动态秩调度参数
            total_step=int(cfg.get("total_step", 125)),
            tinit=int(cfg.get("tinit", 20)),
            tfinal=int(cfg.get("tfinal", 100)),
            deltaT=int(cfg.get("deltaT", 10)),
            orth_reg_weight=float(cfg.get("orth_reg_weight", 0.5)),
        )
...
def make_train_args(cfg):
    # transformers 新旧版本中参数名可能有差异，这里自动判断
    args_dict = {
    ...
    # 新版 transformers 使用 eval_strategy，旧版使用 evaluation_strategy
    ...
    elif "tensorboard_logging_dir" in sig.parameters:
